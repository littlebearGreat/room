...
// 接口
 ...
 $('#index-play').FlexPaperViewer(
        ...
        config : {
          ...
          SWFFile : 'style/Paper.pdf.swf',// 需要使用Flexpaper打开的文档
 ...
 function collect(resourceId){
    var url = path+"/resourceZone/resourceCollect.jspx";
    $.post(url, {"resourceId":resourceId}, function(data) {
        if(data.code == "1"){
            layer.msg(data.message);
            var totalCollect = $("#totalCollectsId").text();
            $("#totalCollectsId").text(parseInt(totalCollect)+1);
            $('#collectTools').html('<a href="javascript:void(0)" id="btn-ysc"></a>');
        }else if(data.code == "0"){
            showLoginFrame();
        }else{
            layer.msg(data.message);
  ...
  * 资源下载
  */
 function resourceDownload(resourceId){
    var url = path+"/resourceZone/resourceHasFile.jspx";
    $.post(url, {"resourceId":resourceId}, function(data) {
        if(data.code == "1"){
             window.location.href = path+"/resourceZone/resourceDownload.jspx?resourceId="+resourceId;
             var downloads = $("#downloadsId").text();
             $("#downloadsId").text(parseInt(downloads)+1);
        }else if(data.code == "0"){
            showLoginFrame();
        }else{
            layer.msg(data.message);
